...
for ds in DEGREES
    for i in 1:NUM_RUNS
        ...
        sol = eliminate_with_love_and_support_modp(ode, 2^31 - 1)
        minpoly_exponents = collect(exponents(sol))

        P = convexhull(minpoly_exponents...)

        minpoly_points_inside = npoints(P)
